...
# Helper functions with probabilties 
def get_predictions(model, X, probabilities):

        if hasattr(model, "boost"):
                X = xgb.DMatrix(X);
        
	if (probabilities is True) and hasattr(model, 'predict_proba'):
		pred = model.predict_proba(X)	
	else:
		pred = model.predict(X)   # throwing an error for xgboost? 
...
class StackedGeneralizer(object):
	...
	def __init__(self,
                     ...
                     probabilities=False,
                     rounds=100,
                     cols_extra=None):     # expecting list of extra columns to include
		"""
			...
			for j, (train_idx, test_idx) in enumerate(kf):
				...
				y_train = y[train_idx]
				        ...
				        self.base_models_cv[model_name].append(copy(model))
                                        ...
                                        bst = xgb.train(model, dTrain, self.rounds, evallist)
                                        self.base_models_cv[model_name].append(copy(bst))
	...
	def transform_base_models(self, X):
		...
		for key in sorted(self.base_models_cv.keys()):
			...
			for i, model in enumerate(self.base_models_cv[key]):
				model_predictions = get_predictions(model, X, self.probabilities)
	...
	def transform_blending_model(self, X_blend):
		...
		for i, model in enumerate(self.blending_model_cv):
			cv_predictions = None
			model_predictions = get_predictions(model, X_blend, self.probabilities)
	...
	def predict(self, X):
		...
		X_blend = self.transform_base_models(X)
                ...
                # Assuming X_blend is a numpy array
                if self.cols_extra is not None and self.cols_extra is ":":
                        X_blend = np.hstack([ X_blend, X ])  
                elif self.cols_extra is not None:                
                        X_blend = np.hstack([ X_blend, X[:,self.cols_extra] ])
